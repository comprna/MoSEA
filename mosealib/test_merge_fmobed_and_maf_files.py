...
def main():
	...
	for input_fmobed_file in input_fmobed_files:
		...
		with open(input_fmobed_file) as fi:	
			...
			for fmofields in fi:
				fmofields = fmofields.strip('\n')	
				fmofields = fmofields.split('\t')

				if len(fmofields) != 10:
					print("Not correct number of fields in fmobed file:{}".format(input_fmobed_file))
					print('\t'.join(fmofields))
					sys.exit()

			 	fmofields_dict.append({"fmo_chr" : fmofields[0],
				...
				#match_fmobed_coordinates(field, maffields_dict, cancer_type, output_file)
	...
	with open(input_maf_file) as fi:
		for line in fi:
			...
			maffields_dict.append({"maf_gene" : maffields[0],
				...
				'''
			match_fmobed_coordinates(line, fmofields_dict, cancer_type, output_file)
...
def match_fmobed_coordinates(line, fmofields_dict, can, outfile):

	fo = open(outfile, "a")

	maffields = line.split('\t')

	maf_gene = maffields[0]
	maf_entrez = maffields[1]
	maf_chr = "chr" + maffields[4]
	maf_chr_start = int(maffields[5])
	maf_chr_end = int(maffields[6])
	...
	maf_sample_id = maf_sample_id.split('-')
	maf_sample_id = maf_sample_id[2] + "T"
	...
	matched_set = [x for x in fmofields_dict if(
							 (x["fmo_chr"] == maf_chr) and
							 (x["fmo_strand"] == maf_chr_strand) and
							 (x["fmo_motif_start"] >= maf_chr_start) and
							 (x["fmo_motif_end"] <= maf_chr_end)) ]

	if len(matched_set):
		outlist_tmp = []
		
		outlist_tmp.extend(can,maf_chr_start,maf_chr_end,maf_variant_class,maf_variant_type,maf_ref_allele,
		maf_tumor_allele1,maf_tumor_allele2,maf_sample_id)

		#outlist_tmp.extend((can,fmo_motif,fmo_eventid,fmo_kmer,fmo_len,fmo_chr,
		...
		for x in matched_set:
			...
			outlist.append(str(b))
			outlist.append(x["fmo_kmer"])
			outlist.append(x["maf_variant_type"])
			outlist.append(x["maf_ref_allele"])
			outlist.append(x["maf_tumor_allele1"])
			outlist.append(x["maf_tumor_allele2"])
			outlist.append(x["maf_sample_id"])
			a = x["maf_sample_id"]
			a = a.split('-')
			sample_id = a[2] + "T"
			outlist.append(sample_id)
